...
void output(int *a, double time, int size);
...
int main(int argc, char* argv[]){
    srand(time(NULL));
    double timeE = 0.0;
    int C = 100; //10 Million element size will cause segment fault, over PC stack size. Stack size is default to 8MB, at 1M elements of ints is 4MB
    int N, R;
    
    for(R = 1, N = C; N >= 10;N /= 10){
        ...
        output(hedgehogs, totalTimeQ, N);
        printf("Iterative Output\n");
        output(iterative, totalTimeI, N);
        printf("Hoare's Output\n");
        output(hoa, totalTimeH, N);
    }
